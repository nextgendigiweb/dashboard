...
const VerifyEmail: React.FC<VerifyEmailProps> = ({ email, onVerify, onCancel }) => {
    const { isLoaded, signUp } = useSignUp();
    const [code, setCode] = useState('');
    const [isVerifying, setIsVerifying] = useState(false);
    const [error, setError] = useState('');
    const [authStep, setAuthStep] = useState('Awaiting_Validation_Token...');
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        setIsVerifying(true);
        setError('');
        setAuthStep('Validating_Neural_Node...');
        ...
        try {
            ...
            if (completeSignUp.status === 'complete') {
                setAuthStep('Validation_Successful_Syncing...');
            ...
            <div className="bg-black border border-white/10 p-4 sm:p-6 md:p-10 relative group shadow-2xl">
                ...
                {isVerifying ? (
                    ...
                    <form onSubmit={handleVerify} className="space-y-4 sm:space-y-5 md:space-y-6">
                        ...
                        >
                            <span className="truncate">Verify_Node</span>
